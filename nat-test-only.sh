...
for nodeName in $(oc get nodes | grep -v NAME | grep "worker-2.shrocp4upi417ovn.lab.upshift.rdu2.redhat.com"| awk {'print $1'}); do
     echo "=========="
     echo $nodeName
     ...
     echo "localpod:"
     echo $localpod
     listOfNats=$(oc -n openshift-ovn-kubernetes exec -it $localpod -c northd -- ovn-nbctl --format=csv --column "_uuid, external_ids, external_ip" find nat)
     for nat in ${listOfNats[@]}; do
        ...
        then
            ...
            echo "test match found - deleting"
            ...
            echo "suuid: ${suuid}"
            echo "slr: ${slr}"
            oc -n openshift-ovn-kubernetes exec -it $localpod -c northd -- ovn-nbctl remove logical_router "${slr}" nat "${suuid}"
            echo "removed nat ${suuid} from logical router ${slr}"

        else
            #echo "match not found, stale nat"
            echo "match not found - normal nat"
            #suuid=$(echo $nat | awk -F ',' {'print $1'})
            #slr=$(oc -n openshift-ovn-kubernetes exec -it $localpod -c northd -- ovn-nbctl --bare --column=_uuid,nat find logical_router | grep -B1 $suuid | awk {'print $1'} | head -n 1)
            #echo "suuid: $suuid"
            #echo "slr: $slr"
