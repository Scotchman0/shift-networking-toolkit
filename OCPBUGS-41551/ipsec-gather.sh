...
for POD in $(oc -n openshift-ovn-kubernetes get pod -o wide | grep -E "$pod1|$pod2" | awk {'print $1'}); do
	...
	oc -n openshift-ovn-kubernetes rsh $POD ip xfrm state > $LOGPATH/$POD_xrfm_state.out
        echo "grabbing ip xrfm policy from $POD"
	oc -n openshift-ovn-kubernetes rsh $POD ip xfrm policy > $LOGPATH/$POD_xrfm_policy.out
        echo "grabbing ip xrfm monitor from $POD"
	# oc -n openshift-ovn-kubernetes rsh $POD ip xfrm monitor > $LOGPATH/$POD_xrfm_monitor.out
        echo "grabbing ipsec status from $POD"
	oc -n openshift-ovn-kubernetes rsh $POD ipsec status > $LOGPATH/$POD_ipsec_status.out
        echo "grabbing ipsec.conf from $POD"
	oc -n openshift-ovn-kubernetes rsh $POD cat /etc/ipsec.conf > $LOGPATH/$POD_ipsec.conf
	...
	oc -n openshift-ovn-kubernetes rsh $POD cat /etc/ipsec.d/cno.conf > $LOGPATH/$POD_cno.conf
        echo "grabbing openshift.conf from $POD"
	oc -n openshift-ovn-kubernetes rsh $POD cat /etc/ipsec.d/openshift.conf > $LOGPATH/$POD_openshift.conf
	...
	oc -n openshift-ovn-kubernetes cp $POD:/tmp/libreswan.tar.gz $LOGPATH/$POD_libreswan.tar.gz
done
