...
DATE=$(date +"%Y-%m-%d-%H-%M")
LOG=${DATE}_healthprobe.out

healthprobe() {
#define CLIENT POD:
POD="client-podname-here"

#iterate over all pods; get the exit result of a connection; if it's a timeout (28) then log the IP of the pod and the result timeout; otherwise skip
#for i in $(oc get pod -n openshift-dns -o wide | grep dns-default | awk {'print $6'}); do RESULT=$(oc -n openshift-dns rsh $POD curl --connect-timeout 1 -kv -s ${i}:5353/health 2>&1 ) ; if [[ $? -eq 7 ]]; then echo "[$i] Timed out"; else dummyvalue=true; fi; echo "."; done
...
for i in $(oc get pod -n openshift-dns -o wide | grep dns-default | awk {'print $6'}); do RESULT=$(oc -n openshift-dns rsh $POD curl --connect-timeout 1 -kv -s ${i}:5353/health 2>&1 ) ; if [[ $? -eq 28 ]]; then echo $i; echo $(oc -n openshift-dns get pod -o wide | grep $i | awk {'print $7}') Timed out ; else dummyvalue=true; fi; echo "."; done | tee $LOG
}
...
#check the list 
for NODE in $(grep ".linux.us.ups.com" $LOG | awk {'print $1'}); do
